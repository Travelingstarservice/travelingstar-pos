...
function renderLogin(errorMessage = ''): void {
  ...
  // Use textContent to set the error message safely — never interpolate
  // server-supplied strings into innerHTML to avoid XSS injection.
  if (errorMessage) {
...
function renderDashboard(username: string): void {
  appRoot.innerHTML = `
    <main class="page">
      <section class="card">
        ...
        <p class="hint">Signed in as <strong id="username-display"></strong>.</p>
  ...
  // Set the username via textContent to prevent XSS from server-supplied values.
  const usernameDisplay = document.querySelector<HTMLElement>('#username-display')
  if (usernameDisplay) {
